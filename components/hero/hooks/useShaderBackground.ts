...
void main(){gl_Position=position;}`
  ...
  constructor(canvas: HTMLCanvasElement, scale: number) {
    ...
    this.shaderSource = this.getBluesTealsShader()
  }

  private getBluesTealsShader(): string {
...
void main(void) {
  ...
  for (float i=1.; i<12.; i++) {
    ...
    vec2 p=uv;
    float d=length(p);
    col+=.00125/d*(cos(sin(i)*vec3(0.2,0.6,0.8))+1.);
  ...
  render(now = 0) {
    ...
    gl.clearColor(0.02, 0.06, 0.23, 1) // Dark slate background
...
export function useShaderBackground() {
  ...
  useLayoutEffect(() => {
    if (!canvasRef.current || isInitializedRef.current) {
      console.log('Skipping WebGL init - canvas:', !!canvasRef.current, 'initialized:', isInitializedRef.current)
      return
    }
    ...
    const initializeWebGL = () => {
      ...
      console.log('Initializing WebGL renderer with DPR:', dpr, 'Canvas:', canvas.width, 'x', canvas.height)
    ...
    const timeoutId = setTimeout(initializeWebGL, 0)
